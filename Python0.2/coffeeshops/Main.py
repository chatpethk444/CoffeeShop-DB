...
def fetch_data(table_name):
    ...
    E_Salary, E_Phone,E_Position, E_ManagerID,DepartmentID
    FROM Employee
    """)
        
     # ถ้าเป็นตาราง Orders ให้คำนวณราคาจาก De_order และ CoffeeBeans (O_Num x B_Price)
    elif table_name == "Orders":
        cursor.execute("""
            SELECT 
                o.OrderID,
                o.O_Date,
				SUM(cb.B_Price * do.O_Num) AS O_Price,
                o.SupplyerID,
                e.EmployeeID
            FROM Orders o
            JOIN De_Order do ON o.OrderID = do.OrderID
            JOIN CoffeeBeans cb ON do.CoffeeBeansID = cb.CoffeeBeansID
            JOIN Supplyer s ON o.SupplyerID = s.SupplyerID
            JOIN Employee e ON o.EmployeeID = e.EmployeeID
            GROUP BY o.OrderID
            """)
        
    # ถ้าเป็นตาราง Receipt ให้คำนวณราคาจาก De_Receipt และ CoffeeBeans (R_num x B_Price)
    elif table_name == "Receipt":
        cursor.execute("""
        SELECT 
            r.ReceiptID,
            r.R_Date,
            SUM(cb.B_Price * dr.R_Num) AS R_Price,
            r.CustomerID,
            r.EmployeeID
        FROM Receipt r
        JOIN De_Receip dr ON r.ReceiptID = dr.ReceiptID
        JOIN CoffeeBeans cb ON dr.CoffeeBeansID = cb.CoffeeBeansID
...
combo_table = Combobox(root, values=["Employee", "CoffeeBeans","Customer", "Department", "Supplyer", "Orders","De_Order", "Receipt","De_Receip"])
